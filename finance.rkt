...
;; Report the team's monthly allocation for the past year
...
(require racket/contract
         ...
         (rename-in gregor
                    ...
                    [date? gregor:date?])
         whatnow/db/types
         whatnow/server/forecast
         whatnow/schedule
         racket/hash
         rackunit
         nocell/sheet/sheet
         nocell/sheet/ods/ods)
...
;;    allocation
(struct/contract allocn  ; Renamed from allocation not to conflict with whatnow
  ([client       string?]
   [project      string?]
...
(define allocation<=? (lexicographic<=? (list allocn-client
                                              allocn-project
                                              allocn-person)
                                        string<?
                                        string=?))
...
(define example-allocations
  (list
   (allocn "Data science for science and the humanities"
           "Interesting Ocean"
           "R-XYZ-001"
           "Oliver Strickson"
           (hash (gregor:date 2020 04) 0.5
                 (gregor:date 2020 05) 0.5))
   (allocn "Data-centric engineering"
           "Ubiquitous Mist"
           "R-ABC-999"
           "Callum Mole"
           (hash (gregor:date 2020 04) 0.5))
    ...
    (sheet
     (list*
      ...
      (for/list ([a (sort allocations allocation<=?)]
                 #:unless
                 (or (member (allocn-client a) '(;; "Corporate Duties"
                                                 ;; "REG Service Areas"
                                                 "Trac days"
                                                 ;; "Turing Programme Support"
                                                 ;; "Turing Service Areas"
                                                 "UNAVAILABLE"))
                     (andmap zero? (hash-values (allocn-month-fraction a)))))

        (list* (cell (allocn-client a) '())
               (cell (allocn-project a) '())
               (cell (allocn-code a) '())
               (cell (allocn-person a) '())
               (map (λ (mon)
                      (cell (hash-ref (allocn-month-fraction a) mon 0.0)
                            '(output)))
                    dates))))))))
...
(define *allocations-combined*
  (map (λ (as)
         (struct-copy allocn
                      (car as)
                      [month-fraction
                       (apply hash-union (map allocn-month-fraction as))]))
       *allocations-grouped*))
